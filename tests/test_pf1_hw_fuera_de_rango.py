...
SI hay una adopcion que el proyectista puede tomar sentado: la carga minima
con que la entrada pasa Q es la energia especifica critica H_c, un numero
que el propio paso 4.2.1 ya calculo. Un metodo «no evaluable» es el que no
se resuelve declarando nada; este se resuelve declarando.

LA MONOTONIA EN D NO ABRE OTRO DIAMETRO, y conviene decirlo porque el
prompt PF-1 lo escribio al reves: HWi/D DECRECE con D (bajan H_c/D y q* a
la vez, con Ks*S fijo), de modo que si la carta se cae en el diametro
minimo del catalogo ninguno mayor la levanta. El descarte del material
...
from src.modelos import (CriterioPendienteError, DisenoNoFactibleError,
                         SeccionCircular, TipoDeBloqueo, TipoMaterial)
