...
segundo eje. Lo invariante es `collected = passed + skipped`, hoy **1801**; lo
...
`1797 passed` **no** demuestra que la suite corra sobre un Python con tkinter
...
independientes. **Las cuatro medidas sobre el mismo árbol en S24.** S22 fue
la primera vez que se midieron las cuatro; desde entonces se vuelven a medir las cuatro cada
vez que el conteo se mueve, que es lo que impide que la tabla vuelva a derivar:
...
El `+1` de S24 sobre las cuatro filas de S22 es **una sola** comprobación
nueva, y no es un test escrito a mano: `test_decisiones_diferidas` está
parametrizado por el campo «Dónde vive» de cada ficha del registro, de modo
que la ficha `S24-01` trajo su caso con ella. Sube igual en las cuatro porque
no depende ni de PyMuPDF ni de tkinter — y aun así se midieron las cuatro, que
es la regla.
