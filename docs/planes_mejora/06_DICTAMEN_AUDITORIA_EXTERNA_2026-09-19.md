...
**Estado del documento.** Cerrado tras la refutación adversarial: 35 refutadores
independientes reprodujeron desde cero los 35 hallazgos nuevos de severidad alta o
media y confirmaron los 35; sus ajustes de severidad y matiz están incorporados.
...
| N-03 | Confirmado | media → **media** | `_NORMA_PRODUCTO` solo por material; M 170M §1.1 y Nota 1 verificados. | No es del todo latente: además de los sitios bajo `informe.dimensionado`, `MD._motivo_descarte` imprime la etiqueta en stdout, JSON y tabla de bloqueos cuando un marco agota su serie («NO es un tope de AASHTO M 170M-04…»). La ausencia de cita de alcance es sistémica a las tres normas de producto (M 170M, M 36, A760: una sola cita cada una, de tablas). La norma de cajón prefabricado sería M 259/M 273 (LRFD 12.4.2.4 y 12.11.1, verificados), ausentes. El proyecto ya decidió «vaciado in situ» en `ruta_familia_c.md` §14.1; es un resto de C7. `Material.norma_producto` es `str` obligatorio: tocar el modelo y tres formateadores. |
...
| V-04 | Confirmado | media → **media** | Ambas pruebas literales. | La ventana emergente entera sí protege (`contenido_de_campo` devuelve `editable=False` y pinta «NO EDITABLE» sin botón); el hueco está en la pestaña 2 de `gui/app.py`, en `--declarar` y en `declaracion.declarar_valor`. Medido: con la tabla pisada a 1.0 el recubrimiento AASHTO de M9 baja de 40.64 a 25.4 mm. `Z_E030` está protegido por accidente (por ser [S]). `test_la_tabla_de_recubrimiento…` afirma en su docstring que el modo impide editar: falso. Solo hay 2 Derivadas y 1 consumidor (M9): la guardia es barata. Z(zona) ejecutable exige transcribir la Tabla N.º 1 de E.030. |
...
- **PC-08 (media)** **Identidad doble de módulos**: `src/` no es paquete y cinco
  ...
  `'503 + 504'`. Los prompts P01–P19 escriben rutas de archivo `src/modulos/...`, que un
  ejecutor puede leer como import (la trampa solo muerde en un proceso mixto donde
  `cli` o `conftest` ya insertaron `src/`, que es el caso de cualquier auditor). Refutación:
...
- **PC-20 (baja, cuantificación de un aviso que la auditoría sí dio)** 34 tests de 15
  archivos (19 M7, 4 M5, 7 M9, 4 `test_cli`) dependen de las declaraciones sintéticas
  de `conftest.py`; los 4 de `test_cli` prueban un pipeline sobre un stub HDPE que el
  producto **nunca puede dimensionar**. Matiz: `test_cierre_perfil` sí ejercita las
  Fases 6 y 7 en proceso sobre concreto; solo la Fase 8 depende del stub. Los otros
  1948 no cambian; la línea base (subproceso sin conftest) sí representa el producto.
...
- **PC-27 (media)** El «estado de verificación» que P01 quiere «formalizar» **ya
  ...
  `PasoDeMemoria` (su `DIFERIDO` tiene cero productores de cálculo), y
  `cli.Bloqueo.diferido_por_alcance` fuera de `modelos.py`. «Fuera de dominio» es una
  bandera sin lector de compuerta que tampoco llega a `informe_json` ni a `cli.volcar`
  (el «NO DEBE USARSE» existe solo en el HTML); «no aplica» no existe.
  `EstabilidadCabezal` tiene tres asserts en `test_M9_cabezal` y ningún constructor
  de producción produce la tupla vacía.
