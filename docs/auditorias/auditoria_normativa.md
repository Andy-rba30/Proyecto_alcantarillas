...
|---|---|---|---|---|
| `temario_refutar_95.json` | 95 | 64 | 31 | `R95-065` |
...
Ítems reabiertos y cerrados tras el corte de sesión: **73**. Se sostienen **34** · ajustados **37** · refutados **2** · fuera de alcance **0**.
...
| `R95-060` | `H-05` | **AJUSTADO** | MEDIA | CONTRADICE | AASHTO LRFD Bridge Design Specifications, 9th ed. (2020), Tabla 3.4.1-2, pag. impresa 3-18 (PDFPAGE 72), leida como imagen del PDF original; Manual de Puentes MTC, Tabla 2.4.5.3.1-2, pag. impresa 143 (PDFPAGE 144) | La frase es efectivamente falsa para la unica fila que le corresponde al cabezal de Fase 9: «Retaining Walls and Abutments» da minimo 1.00, y es la unica fila del bloque EV con ese minimo. Hasta ahi el hallazgo se sostiene. Pero la severidad CRITICA no: el par {1.35, 0.90} no es un numero mal transcrito -- «Rigid Frames 1.35 / 0.90» es una fila literal de la tabla -- y el unico consumidor ejecutable del criterio es M8_estructural.factores_carga_flotacion() (V7, conducto enterrado), donde 0.90 es el minimo correcto. Verifique ademas que M9_cabezal.factores_de_carga() y combinaciones() NO tienen ningun llamador en src/modulos/ (solo docstrings y tests), y que volteo/deslizamiento del cabezal se evaluan con los FS de Sec. 9.3 (E.050) sin aplicar ningun gamma: cae, por tanto, la agravante del auditor de que «el codigo minora un 10 % de mas» en M9 y la de la presion de contacto. Queda un defecto declarativo real -- un unico par EV que no puede expresar la dependencia por tipo de estructura, con una frase de justificacion desmentible en la pag. 3-18 -- que es MEDIA, y coincide con lo ya registrado en el propio repo (auditoria_normativa.md:1507, R48-031, AJUSTADO/MEDIA). |
| `R95-061` | `H-06` | **SE SOSTIENE** | ALTA | CONTRADICE | AASHTO LRFD Bridge Design Specifications, 9th ed. (2020), Tabla 3.4.1-2, bloque EH, pag. impresa 3-18 (PDFPAGE 72); Manual de Puentes MTC, Tabla 2.4.5.3.1-2, pag. impresa 143 (PDFPAGE 144) | Es una afirmacion negativa sobre la fuente y es falsa sin matiz posible: la tabla SI declara minimo para el empuje en reposo, 0.90, en las dos ediciones disponibles (AASHTO 9a ed. y su transcripcion del MTC). El «N/A» que el repo se atribuye pertenece a la fila siguiente, «AEP for anchored walls»; el error es un desfase de una linea al emparejar las columnas Maximum y Minimum de un bloque de tres filas. Busque un matiz en el resto del criterio y no lo hay: el unico comentario adyacente (:1345, «informativo: el proyecto disena con empuje activo, no en reposo») justifica no usar el valor, no la afirmacion sobre lo que la fuente declara. Mantengo ALTA y no subo a CRITICA porque el valor no entra en ningun calculo -- ningun modulo de src/modulos/ lee 'EH_en_reposo' -- y el propio auditor ya ponderó ese caracter informativo al proponer la severidad. |
| `R95-062` | `H-07` | **SE SOSTIENE** | ALTA | CONTRADICE | AASHTO LRFD 9th ed., pags. impresas 3-14, 3-17 y 3-18 (PDFPAGE 68, 71 y 72); Manual de Puentes MTC, pags. impresas 143, 144 y 146 (PDFPAGE 144, 145 y 147) | Dos de las cuatro paginas citadas son falsas y las comprobe una por una. La 3.4.1-1 esta en 3-17, tres paginas despues de la 3-14 que declara el campo `fuente`; y la pag. 146 del Manual de Puentes no solo no trae la Tabla 2.4.5.3.1-2 sino que no trae texto normativo alguno, es una foto de portadilla. No hay confusion pagina impresa/pagina PDF que salve la cita: con el offset del briefing (impresa = pdfpage - 1) la 143 y la 146 del Manual siguen dando PDFPAGE 144 y 147. Encaja exactamente en «pagina equivocada» de la escala del briefing, sin agravantes ni atenuantes: la severidad ALTA propuesta es la correcta. La correccion es «Tablas 3.4.1-1 (pag. 3-17) y 3.4.1-2 (pag. 3-18); Manual de Puentes, Tablas 2.4.5.3.1-1 y 2.4.5.3.1-2, ambas en pag. 143». |
| `R95-063` | `H-08` | **AJUSTADO** | ALTA | CONTRADICE | Verificacion INTERNA del repo (no hay PDF): docs/manifiesto_citas.md, seccion 8 «AASHTO LRFD Bridge Design Specifications», lineas 286-288 y filas 295, 298, 299 y 300, contra src/criterios_adoptados.py en el mismo SHA (git diff 71b134f..HEAD sobre ambos archivos: vacio) | El fondo se sostiene entero: el preambulo de la seccion 8 describe un estado del codigo que ya no existe y las cuatro filas invierten valor y etiqueta ([A] donde el codigo declara [C]). No hay atenuante de «foto historica»: la misma seccion 8 sabe tachar lo retirado (la fila de `clase_sitio` va con «CITA FALSA — RETIRADA») y otras secciones ya se actualizaron con «cita cerrada» (§7 HDS-5, §10-bis WSDOT), asi que la omision es selectiva. La consecuencia esta demostrada en este mismo lote: quien use el manifiesto como indice se salta justo las filas donde viven los defectos de R95-060 y R95-061. Lo que AJUSTO son las referencias del propio hallazgo: las cuatro filas estan en las lineas 295, 298, 299 y 300, no en 296, 301, 302 y 303 -- la 296 es la fila de `FS_flotacion`, la 301 es `punto_aplicacion_incremento_sismico`, que comprobe en src/criterios_adoptados.py:579-580 y SI sigue siendo `valor=None` con `etiqueta="A"` (es decir, esa linea citada es correcta), la 302 es la advertencia transversal y la 303 esta en blanco. Severidad ALTA confirmada, en linea con el registro interno del propio repo (auditoria_normativa.md:117, MAN-03). |
| `R95-064` | `H-09` | **AJUSTADO** | ALTA | CONTRADICE | Verificacion INTERNA del repo (no hay PDF): docs/manifiesto_citas.md linea 296, contra src/criterios_adoptados.py:842, :848 y :1215-1228 y src/modulos/M8_estructural.py:52 y :72 | El hallazgo se sostiene: el manifiesto inventaria como [C] un criterio que el codigo declara retirado, con un ancla que aterriza en territorio ajeno. Subo la severidad de MEDIA a ALTA porque no es solo una referencia archivo:linea desfasada -- que seria MEDIA en la escala del briefing -- sino un item anunciado al revisor que no existe, y la seccion 8 tiene convencion propia para retirar filas («CITA FALSA — RETIRADA» en la de `clase_sitio`) que aqui no se aplico; el registro interno del repo ya lo tenia en ALTA (auditoria_normativa.md:117, MAN-03). Ajusto dos precisiones del auditor: la fila esta en la linea 296 del manifiesto, no en la 297 (la 297 es «Rigidez de anillo, pandeo y resistencia de costura»), y src/criterios_adoptados.py:842 no es el criterio `geometria_control_salida` -- que empieza en la linea 848 -- sino el cierre del campo `fuente` del criterio `ke_entrada`, tambien de HDS-5, lo que no cambia el fondo: el ancla cae en otro criterio de otro documento normativo. |
...
**`R95-060` — hallazgo `H-05` — AJUSTADO (MEDIA, CONTRADICE)**

*Dónde se miró:* AASHTO LRFD Bridge Design Specifications, 9th ed. (2020), Tabla 3.4.1-2, pag. impresa 3-18 (PDFPAGE 72), leida como imagen del PDF original; Manual de Puentes MTC, Tabla 2.4.5.3.1-2, pag. impresa 143 (PDFPAGE 144)

Abri el PDF en la pag. 72 (cabecera «3-18 AASHTO LRFD BRIDGE DESIGN SPECIFICATIONS, NINTH EDITION, 2020») y el bloque EV da, fila por fila: «Overall and Compound Stability 1.00 / N/A», «Retaining Walls and Abutments 1.35 / 1.00», «Rigid Buried Structure 1.30 / 0.90», «Rigid Frames 1.35 / 0.90», «Metal Box Culverts... 1.50 / 0.90», «Thermoplastic Culverts 1.30 / 0.90», «All others 1.95 / 0.90». Identico en MTC_PUENTES PDFPAGE 144: «Muros y estribos de retencion. 1.35 / 1.00». En el repo lei src/criterios_adoptados.py:1343 («"EV": {"max": 1.35, "min": 0.90},») y :1379-1380 («la tabla fuente da EV minimo 0.90, no 1.00»).

**`R95-061` — hallazgo `H-06` — SE SOSTIENE (ALTA, CONTRADICE)**

*Dónde se miró:* AASHTO LRFD Bridge Design Specifications, 9th ed. (2020), Tabla 3.4.1-2, bloque EH, pag. impresa 3-18 (PDFPAGE 72); Manual de Puentes MTC, Tabla 2.4.5.3.1-2, pag. impresa 143 (PDFPAGE 144)

En la imagen del PDF (pag. 72, cabecera «3-18») el bloque EH lista tres filas con sus dos columnas completas: «Active 1.50 / 0.90», «At-Rest 1.35 / 0.90», «AEP for anchored walls 1.35 / N/A». El Manual de Puentes lo repite en PDFPAGE 144: «EH: Presion Horizontal de la tierra. / Activa. 1.50 / 0.90 / En reposo. 1.35 / 0.90 / AEP Para paredes ancladas. 1.35 / N/A». En el repo lei src/criterios_adoptados.py:1380-1382 («La Tabla 3.4.1-2 ademas distingue EH activo (1.50/0.90) de EH en reposo (1.35, sin minimo declarado por la fuente)») y :1345 («"EH_en_reposo": {"max": 1.35},»).

**`R95-062` — hallazgo `H-07` — SE SOSTIENE (ALTA, CONTRADICE)**

*Dónde se miró:* AASHTO LRFD 9th ed., pags. impresas 3-14, 3-17 y 3-18 (PDFPAGE 68, 71 y 72); Manual de Puentes MTC, pags. impresas 143, 144 y 146 (PDFPAGE 144, 145 y 147)

Abri el PDF de AASHTO en la pag. 71: cabecera «SECTION 3: LOADS AND LOAD FACTORS 3-17» y, entera en esa pagina, «Table 3.4.1-1—Load Combinations and Load Factors»; la pag. 72 lleva «Table 3.4.1-2—Load Factors for Permanent Loads». La pag. impresa 3-14 (PDFPAGE 68) no tiene tabla alguna: es articulado/comentario («The larger of the two values provided for load factor of TU shall be used for deformations...»). En MTC_PUENTES, PDFPAGE 144 (= impresa 143) contiene AMBAS, «Tabla 2.4.5.3.1-1 Combinaciones de Carga y Factores de Carga» y «Tabla 2.4.5.3.1-2 Factores de carga para cargas permanentes»; la impresa 146 (PDFPAGE 147) es una lamina fotografica: «PUENTE. CARACHA». En el repo lei src/criterios_adoptados.py:1390-1392: «Tablas 3.4.1-1 (pag. 3-14) y 3.4.1-2 (pag. 3-18); transcritas tambien en Manual de Puentes MTC, pags. 143 y 146».

**`R95-063` — hallazgo `H-08` — AJUSTADO (ALTA, CONTRADICE)**

*Dónde se miró:* Verificacion INTERNA del repo (no hay PDF): docs/manifiesto_citas.md, seccion 8 «AASHTO LRFD Bridge Design Specifications», lineas 286-288 y filas 295, 298, 299 y 300, contra src/criterios_adoptados.py en el mismo SHA (git diff 71b134f..HEAD sobre ambos archivos: vacio)

Lei docs/manifiesto_citas.md:286-288: «Ninguna cita a AASHTO LRFD lleva hoy un valor numerico transcrito: **todas las tablas quedaron sin extraer**. Este bloque es, casi entero, el inventario de lo que falta traer del PDF.» Y en el codigo: :1339-1343 `factores_carga_aashto` con el dict completo de gammas y `etiqueta="C"`; :1401-1402 `peso_especifico_concreto_kn_m3` con `valor=23.56` y `etiqueta="C"`; :1501-1503 `recubrimiento_aashto_mm` con `{"contra_suelo": 75.0, ...}` y `etiqueta="C"`; :1586-1601 `procedimiento_flexion_corte_aashto_sec5` con phi, beta, theta, Vc, Vs, s y dv y `etiqueta="C"`. Las cuatro filas del manifiesto los siguen listando como «= None» con etiqueta «[A]». El propio manifiesto se define en su cabecera como «un volcado literal de lo que el codigo YA afirma».

**`R95-064` — hallazgo `H-09` — AJUSTADO (ALTA, CONTRADICE)**

*Dónde se miró:* Verificacion INTERNA del repo (no hay PDF): docs/manifiesto_citas.md linea 296, contra src/criterios_adoptados.py:842, :848 y :1215-1228 y src/modulos/M8_estructural.py:52 y :72

docs/manifiesto_citas.md:296 dice literal: «| `FS_flotacion = None` — FS de V7, ΣW ≥ FS·U | Sec. 12 (no incorporada por el Manual de Puentes) | AASHTO LRFD | [CA:842](src/criterios_adoptados.py:842) | [C] |». En el codigo, src/criterios_adoptados.py:1215-1217 declara: «# 'FS_flotacion' SE RETIRO. Declaraba el factor de seguridad global de ΣW >= FS*U, que es lenguaje de TENSION ADMISIBLE, y V7 se reescribio como el equilibrio de factores de carga que corresponde al marco LRFD». Un grep de `FS_flotacion` sobre src/ no devuelve ninguna clave viva del dict CRITERIOS, y tests/test_M8_estructural.py:130 asegura precisamente «assert "FS_flotacion" not in ca.CRITERIOS».
