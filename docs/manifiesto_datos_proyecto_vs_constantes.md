...
| 3 | `factor_muro = 1.0` | `criterios_adoptados.py` | `[N]` | ↻↻ **El reparto era el correcto y la palabra «tabla» era falsa** (`NOR-PUE-07`). El num. 2.8.1.1.14.2.2 **no presenta ninguna tabla** —en las págs. impresas 252-257 del Manual no hay una sola— y fija **un** valor, `REDUCCION_KH_POR_DESPLAZAMIENTO = 0.5`, y de forma **permisiva**: «k_h *puede* ser reducido a 0.5k_h0». El 1.0 no era una fila: es la **ausencia** de reducción, que es la definición misma de k_h0 («asumiendo que el desplazamiento del muro sea cero»). Hoy: el 0.5 en [`constantes_normativas.py`](../src/constantes_normativas.py) y, en [`criterios_adoptados.py`](../src/criterios_adoptados.py), `factor_muro_eleccion = "sin_reduccion"`, que **no elige una fila sino que declara si se aplica la reducción** | `[N]` el 0.5, `[A]` la declaración | Sigue siendo el mismo reparto de `F_pga` y `v_max_concreto_eleccion`, pero con el vocabulario que la fuente admite. La justificación es la ya escrita («cabezal empotrado, sin desplazamiento admisible garantizado») y AASHTO añade una condición que **no es técnica**: el movimiento tiene que ser aceptable para el propietario, lo que deja la reducción fuera del alcance de un cálculo. `M9.factor_muro()` rechaza con `DatoInvalidoError` cualquier declaración que no sea una de las dos |
