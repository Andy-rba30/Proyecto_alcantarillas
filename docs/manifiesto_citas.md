...
| `N_cq_N_gammaq_meyerhof = None` — salen de **figuras**, no de tabla ni fórmula | figuras 2.8.1.3.1.2c-1 y 2.8.1.3.1.2c-2 (Meyerhof 1957), págs. 272-273 | Manual de Puentes | [CA:2179](src/criterios_adoptados.py:2179) | [A] |
| `factores_carga_aashto = None` — el Manual nombra las combinaciones y no transcribe las Tablas 3.4.1-1/-2 | 2.4.5.3, págs. 140-143 | Manual de Puentes (vía AASHTO) | [CA:2070](src/criterios_adoptados.py:2070) | [A] |
| `procedimiento_flexion_corte_aashto_sec5 = None` — remisión a AASHTO Sec. 5 | Manual de Puentes Sección 2.9, pág. 337 | Manual de Puentes (vía AASHTO) | [CA:2317](src/criterios_adoptados.py:2317), [M9:170](src/modulos/M9_cabezal.py:170), [M9:1333-1334](src/modulos/M9_cabezal.py:1333) | [A] |
...
| `metodo_estabilidad_global = None` — **afirmación negativa**: E.050 fija el umbral, no el método | Art. 30.3 y num. 39.13.6 b) | E.050 | [CA:2210](src/criterios_adoptados.py:2210) | [A] |
...
| ↻ **Cita literal de lo que sí fija** — «La altura de relleno mínimo desde la clave de la tubería hasta el nivel de la subrasante será de 0,30 m.» Es exactamente la magnitud que calcula V7 (subrasante − clave) | Subsección 508.07, pág. 982 | EG-2013 | [CN:208](src/constantes_normativas.py:208) | [N] |
...
| ↻ `factores_carga_aashto` — factores γ (DC, EV, EH, LS, WA, EQ) con máximos y mínimos. **TIENE VALOR**: diccionario completo por combinación. Decía `= None` `[A]` | Tablas 3.4.1-1 y 3.4.1-2 | AASHTO LRFD | [CA:2070](src/criterios_adoptados.py:2070), [M9:52](src/modulos/M9_cabezal.py:52), [M9:805](src/modulos/M9_cabezal.py:805) | **[C]** |
| ~~`FS_flotacion = None` — FS de V7, ΣW ≥ FS·U~~ **CRITERIO RETIRADO**, y esta fila lo inventariaba como vivo con un ancla que caía en otro criterio (NOR-MAN-03). V7 se reescribió como equilibrio de factores de carga LRFD (§0.2, Fase 5 V7): un FS global es lenguaje de tensión admisible y conservarlo además de los γ contaría dos veces el mismo margen. El código lo declara retirado y un test lo vigila | — | — | [CA:1869](src/criterios_adoptados.py:1869), [M8:63](src/modulos/M8_estructural.py:63), [test_M8:130](tests/test_M8_estructural.py:130) | ~~[C]~~ → **retirado** |
...
| ↻ `recubrimiento_aashto_mm` — lado AASHTO de la regla "rige el recubrimiento MAYOR". **TIENE VALOR**: 75 mm en las tres condiciones. Decía `= None` `[A]`. Su número está en revisión por otra vía (categoría de refuerzo y modificador por a/c), que es un asunto distinto de esta fila | tabla de recubrimientos mínimos, Sec. 5 | AASHTO LRFD | [CA:2232](src/criterios_adoptados.py:2232), [M9:1170-1173](src/modulos/M9_cabezal.py:1170) | **[C]** |
| ↻ `procedimiento_flexion_corte_aashto_sec5` — factores φ, límites de refuerzo, modelo de corte (MCFT / β-θ). **TIENE VALOR**: φ, modelo de corte y β-θ declarados. Decía `= None` `[A]` | Sección 5 (vía Manual de Puentes Sec. 2.9, pág. 337) | AASHTO LRFD | [CA:2317](src/criterios_adoptados.py:2317), [M9:1333-1345](src/modulos/M9_cabezal.py:1333) | **[C]** |
| ↻ `peso_especifico_concreto_kn_m3` — peso unitario del concreto armado. **TIENE VALOR**: 23.56 kN/m³. Decía `= None` `[A]` | Tabla 3.5.1-1 (o Manual de Puentes) | AASHTO LRFD | [CA:2132](src/criterios_adoptados.py:2132) | **[C]** |
...
| `talud_terraplen = None` | **DG-2018** y sección tipo del proyecto | [CA:1977-1260](src/criterios_adoptados.py:1977) | [A] |
...
| `longitud_proteccion_salida = None` | Práctica corriente de enrocado o **HEC-14** | [CA:2026-1308](src/criterios_adoptados.py:2026) | [A] |
...
| `espesor_proteccion_salida = 1.75`·d50 | "Práctica corriente de diseño de enrocado" (rango 1.5–2.0 d50) | [CA:2015](src/criterios_adoptados.py:2015) | [A] |
| `angulo_aletas = None` | "Práctica corriente; no fijado por el Manual" | [CA:2044](src/criterios_adoptados.py:2044) | [A] |
| `N_cq_N_gammaq_meyerhof = None` | **Meyerhof (1957)**, vía las figuras del Manual de Puentes (ver §3) | [CA:2179-1449](src/criterios_adoptados.py:2179) | [A] |
...
| `peso_especifico_relleno_kn_m3` | `None` | **vacío — bloquea el término ΣW de V7** | (17.0, 20.0) kN/m³ | [CA:1946](src/criterios_adoptados.py:1946) |
| `talud_terraplen` | `None` | **vacío — bloquea la longitud del conducto en 7.B** | — | [CA:1977](src/criterios_adoptados.py:1977) |
| `espesor_proteccion_salida` | 1.75·d50 | declarado | (1.5, 2.0) | [CA:2015](src/criterios_adoptados.py:2015) |
| `longitud_proteccion_salida` | `None` | **vacío — completa el diseño de la Fase 6** | — | [CA:2026](src/criterios_adoptados.py:2026) |
| `angulo_aletas` | `None` | **vacío** | — | [CA:2044](src/criterios_adoptados.py:2044) |
...
| ⚠ `factores_carga_aashto` | **γ por combinación (dict completo)** | **NO es `[A]`: es `[C]` y TIENE valor.** Fila conservada, corregida, para quien la buscara aquí. Se verifica como cita en §8 | — | [CA:2070](src/criterios_adoptados.py:2070) |
| ⚠ `peso_especifico_concreto_kn_m3` | **23.56 kN/m³** | **NO es `[A]`: es `[C]` y TIENE valor.** Ídem | (23.5, 24.5) kN/m³ | [CA:2132](src/criterios_adoptados.py:2132) |
| `predimensionamiento_cabezal` | `None` | **vacío — bloquea la estabilidad automática** | — | [CA:2151](src/criterios_adoptados.py:2151) |
| `N_cq_N_gammaq_meyerhof` | `None` | **vacío — bloquea la capacidad portante en talud** | — | [CA:2179](src/criterios_adoptados.py:2179) |
| `metodo_estabilidad_global` | `None` | **vacío — bloquea E4 y E5 de Sec. 9.3** | — | [CA:2210](src/criterios_adoptados.py:2210) |
| ⚠ `recubrimiento_aashto_mm` | **75 mm en las tres condiciones** | **NO es `[A]`: es `[C]` y TIENE valor.** Ídem. Su número está en revisión por otra vía (AASHTO da 3.0 in = 76.2 mm para acero sin recubrir, y con galvanizado o epóxico daría 50.8; falta además el modificador por a/c). Eso NO es este hallazgo | — | [CA:2232](src/criterios_adoptados.py:2232) |
| `cortante_alto_muro_e060_art_11_10_10_2` | `None` | **vacío — bloquea el escalón de ρ a 0.0025 (E.060 Art. 11.10.10.2)** | — | [CA:2265](src/criterios_adoptados.py:2265) |
| ⚠ `procedimiento_flexion_corte_aashto_sec5` | **φ, modelo de corte y β-θ declarados** | **NO es `[A]`: es `[C]` y TIENE valor.** Ídem | — | [CA:2317](src/criterios_adoptados.py:2317) |
