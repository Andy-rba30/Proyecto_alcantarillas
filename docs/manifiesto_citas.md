...
> (`NUMERAL_V6`, [M5:328](src/modulos/M5_verificaciones.py:328)). Ver §11.
...
| `NUMERAL_V4` — `ReferenciaNormativa`: `seccion_hoja_ruta="Sec. 5.1"` / `numeral_norma="Manual de Suelos…, num. 4.5.4 y 9.1(3)"` | 4.5.4 y 9.1(3) | Manual de Suelos | [M5:295](src/modulos/M5_verificaciones.py:295) | [N→] |
| `resguardo_por_cbr()` — tabla de CBR aplicada en V4 | num. 4.5.4 | Manual de Suelos | [M5:1220](src/modulos/M5_verificaciones.py:1220) | [N] |
...
| `NUMERAL_V7 = "Fase 5, V7 (Manual de Puentes num. 2.4.3.8.2 + Fase 8, item 3)"` | 2.4.3.8.2 | Manual de Puentes | [M5:329](src/modulos/M5_verificaciones.py:329), [M8:193](src/modulos/M8_estructural.py:193) | [N] |
...
| `procedimiento_flexion_corte_aashto_sec5 = None` — remisión a AASHTO Sec. 5 | Manual de Puentes Sección 2.9, pág. 337 | Manual de Puentes (vía AASHTO) | [CA:5432](src/criterios_adoptados.py:5432), `diseno_flexion_corte` [M9:3279](src/modulos/M9_cabezal.py:3279) | [A] |
...
| ✚ `factor_recubrimiento_banda_intermedia_ac = 1.0` — **el hueco del corpus peruano, ahora declarado como `[C]` en vez de vivir como literal en el módulo**. El Manual imprime **dos** factores (0.8 y 1.2) y deja sin factor la banda 0.40 &lt; a/c &lt; 0.50; AASHTO sí la cubre. Multiplicar por uno es neutro en aritmética y **no** en normativa: aquí significa «en esta banda no se modifica el recubrimiento», que es una afirmación que el corpus peruano no hace. Y la banda **es alcanzable** — con sulfatos severos y sin cloruros la a/c máxima resulta 0.45 | Art. 5.10.1, pág. impresa 5-167 | AASHTO LRFD 9.ª ed. | [CA:5124](src/criterios_adoptados.py:5124) | [C] |
...
| `c_phi_fundacion = None` — la obligación de usar solo uno (φ=0 o c=0) | Art. 20 | E.050 | [CA:2757](src/criterios_adoptados.py:2757) | [A] |
...
| `metodo_estabilidad_global = None` — **afirmación negativa**: E.050 fija el umbral, no el método | Art. 30.3 y num. 39.13.6 b) | E.050 | [CA:4977](src/criterios_adoptados.py:4977) | [A] |
...
| **Afirmación negativa**: las Secciones 505, 506 y 507 solo regulan colocación y compactación y remiten a la Sección 502, que tampoco fija altura mínima de diseño — lo declara `cobertura_minima_aashto` | 505 / 506 / 507 → 502 | EG-2013 | [CA:3551](src/criterios_adoptados.py:3551) | [C] |
| **Remisiones formales** que cierran el circuito hacia §9: 506.02 → AASHTO M-170M (concreto reforzado); 507.05 / 507.06 / 507.08 → ASTM A-807 (TMC). ⚠ Esta remisión del EG-2013 a **A-807 es correcta y es de materiales y fabricación**; lo que era falso es atribuirle a A-807 la tabla de **calibre por altura de cobertura**, que es de ASTM A796/A796M (`NOR-PRO-04`) — lo declara `clases_producto_por_relleno` | 506.02, pág. 959; 507.05/.06 **pág. 973** y 507.08 **pág. 974** (verificadas leyendo el PDF; decían 969-970) | EG-2013 → norma de producto | [CA:4306](src/criterios_adoptados.py:4306) | [C] |
...
| ↻ **`recubrimiento_aashto_mm` fue RETIRADO como criterio** (cluster C07). Era el lado AASHTO de la regla «rige el recubrimiento MAYOR», con valor declarado —75 mm en las tres condiciones— y la conclusión ya escrita: «AASHTO gobierna en los tres casos». Los 75 mm eran los 3.0 in de la **columna A** redondeados a la baja (`MAT-D16`), sin decir que la tabla tiene **tres** columnas (`NOR-AAS-01`) y sin el modificador por a/c (`NOR-AAS-05`). Hoy el lado AASHTO **se calcula** en `M9.recubrimiento_de_diseno` —tabla × factor por a/c, con el piso de 1.0 in— y las tres piezas viven separadas: la tabla en `tabla_recubrimiento_aashto_mm` `[C]`, la columna en `categoria_refuerzo_aashto` `[A]` **vacío**, y el emparejamiento con las filas de E.060 en `situacion_recubrimiento_aashto` `[A]` | Tabla 5.10.1-1, pág. impresa 5-169 (PDF 528) | AASHTO LRFD 9.ª ed. | [CA:5040](src/criterios_adoptados.py:5040), [M9:2677](src/modulos/M9_cabezal.py:2677) | ~~[C]~~ → **retirado** |
| ✚ `tabla_recubrimiento_aashto_mm` — la **Tabla 5.10.1-1 completa**: 21 situaciones × 3 categorías de material de refuerzo, en mm. Se transcribe entera aunque el cálculo use dos filas, porque un solo número ya digerido no dejaba ver ni las tres columnas ni el modificador. Nota al pie: «*Category A — Uncoated reinforcing steel meeting AASHTO M 31M/M 31 · Category B — Epoxy coated or galvanized meeting ASTM A775/A775M · Category C — Materials meeting AASHTO M 334M/M 334*». Es `[C]` y no `[N]` porque cubre lo que el corpus peruano **no** tabula: las columnas B y C | Tabla 5.10.1-1, pág. impresa 5-169 (PDF 528); Art. 5.10.1, pág. impresa 5-167 (PDF 526) | AASHTO LRFD 9.ª ed. | [CA:5304](src/criterios_adoptados.py:5304) | **[C]** |
| ✚ `categoria_refuerzo_aashto = None` — **la condición de aplicación que el expediente nunca declaró** (`NOR-AAS-01`). Con categoría A la tabla da 3.0 in y con B o C, 2.0 in: la elección mueve el recubrimiento adoptado y mueve **quién lo impone**. La fija la especificación del acero que se compra, no una lectura de norma, y por eso es un vacío que detiene el cálculo | Tabla 5.10.1-1 y su nota al pie, pág. impresa 5-169 | AASHTO LRFD 9.ª ed. | [CA:5040](src/criterios_adoptados.py:5040) | [A] |
| ↻ `procedimiento_flexion_corte_aashto_sec5` — factores φ, límites de refuerzo, modelo de corte (MCFT / β-θ). **TIENE VALOR**: φ, modelo de corte y β-θ declarados. Decía `= None` `[A]` | Sección 5 (vía Manual de Puentes Sec. 2.9, pág. 337) | AASHTO LRFD | [CA:5432](src/criterios_adoptados.py:5432), `diseno_flexion_corte` [M9:3279](src/modulos/M9_cabezal.py:3279) | **[C]** |
| ↻ `peso_especifico_concreto_kn_m3` — peso unitario del concreto armado. **TIENE VALOR**: 23.56 kN/m³. Decía `= None` `[A]` | Tabla 3.5.1-1 (o Manual de Puentes) | AASHTO LRFD | [CA:4861](src/criterios_adoptados.py:4861) | **[C]** |
...
| ↻ `diametros_normalizados` = inicio 0.90, paso 0.15. **Los topes salieron de aquí** y viven en `D_max_catalogo`: el paso sí se verifica contra la serie de diámetros nominales de las normas de producto, y los topes no salen de ninguna | series de diámetro nominal: A760 Tabla 1 (150→3600 mm en escalones de 150 desde 900); M 170M Tablas 1-5 (300→3600 mm); M294 (serie de 150 mm) | ASTM / AASHTO | [CA:2862](src/criterios_adoptados.py:2862) | [C] |
| ↳ verificación pendiente declarada, **ahora en `D_max_catalogo`**: los tres topes siguen sin respaldo documental, y el de HDPE es el único cuya norma (AASHTO M294) ni siquiera está en `normas/` para contrastarlo | — | ninguno | [CA:3422](src/criterios_adoptados.py:3422) | [A] |
...
| `demanda_sismica_licuefaccion = 1000` años — **descarta** el sismo de 475 años de E.030 por coherencia con el Manual de Puentes | (referencia al Tr 475 de E.030) | E.030 (descartado) | [CA:2831](src/criterios_adoptados.py:2831) | [A] |
...
| `NUMERAL_V6` | `"3.1"` | [M5:328](src/modulos/M5_verificaciones.py:328) |
| `NUMERAL_V8` | `"Fase 5, V8"` — "[N] verificación, no diseño, sin numeral que fije el TR ni el umbral": el TR y el umbral que faltan son los del criterio `TR_evento_extremo`, que `M5.v8_evento_extremo` invoca y que sigue vacío | [M5:334](src/modulos/M5_verificaciones.py:334), [CA:2711](src/criterios_adoptados.py:2711) |
| `NUMERAL_V9` | `"Sec. 3.2 (V9, nuevo en v7)"` | [M5:335](src/modulos/M5_verificaciones.py:335) |
...
|---|---|---|---|
| `remanso_derecho_via = None` | Manual de Diseño Geométrico **DG-2018** + **Ley 29338**; requiere perfil de remanso (paso a paso o HEC-RAS) | [CA:2608](src/criterios_adoptados.py:2608), [M5:338](src/modulos/M5_verificaciones.py:338) | [A] |
| `talud_terraplen = None` | **DG-2018** y sección tipo del proyecto | [CA:4440](src/criterios_adoptados.py:4440) | [A] |
...
| `longitud_proteccion_salida = None` | Práctica corriente de enrocado o **HEC-14** | [CA:4540](src/criterios_adoptados.py:4540) | [A] |
...
| `Mw_licuefaccion = None` | Desagregación del peligro sísmico / sismo de subducción del norte peruano | [CA:2803](src/criterios_adoptados.py:2803) | [A] |
...
| `espesor_proteccion_salida = 1.75`·d50 | "Práctica corriente de diseño de enrocado" (rango 1.5–2.0 d50) | [CA:4523](src/criterios_adoptados.py:4523) | [A] |
| `angulo_aletas = None` | "Práctica corriente; no fijado por el Manual" | [CA:4575](src/criterios_adoptados.py:4575) | [A] |
| `N_cq_N_gammaq_meyerhof = None` | **Meyerhof (1957)**, vía las figuras del Manual de Puentes (ver §3) | [CA:4929](src/criterios_adoptados.py:4929) | [A] |
...
| `remanso_derecho_via` | `None` | **vacío — bloquea V5 para todo punto** | — | [CA:2608](src/criterios_adoptados.py:2608) |
| `TR_evento_extremo` | `None` | **vacío — bloquea V8 para todo punto** | — | [CA:2711](src/criterios_adoptados.py:2711) |
| `phi_relleno_trasdos` | `None` | **vacío** | (30.0, 38.0) ° | [CA:2739](src/criterios_adoptados.py:2739) |
| `c_phi_fundacion` | `None` | **vacío** | — | [CA:2757](src/criterios_adoptados.py:2757) |
| `capacidad_portante_adm` | `None` | **vacío** | — | [CA:2783](src/criterios_adoptados.py:2783) |
| `Mw_licuefaccion` | `None` | **vacío — bloquea la evaluación de licuefacción** | — | [CA:2803](src/criterios_adoptados.py:2803) |
| `demanda_sismica_licuefaccion` | 1000 años | declarado | (475, 1000) | [CA:2831](src/criterios_adoptados.py:2831) |
| `peso_especifico_relleno_kn_m3` | `None` | **vacío — bloquea el término ΣW de V7** | (17.0, 20.0) kN/m³ | [CA:4397](src/criterios_adoptados.py:4397) |
| `talud_terraplen` | `None` | **vacío — bloquea la longitud del conducto en 7.B** | — | [CA:4440](src/criterios_adoptados.py:4440) |
| `espesor_proteccion_salida` | 1.75·d50 | declarado | (1.5, 2.0) | [CA:4523](src/criterios_adoptados.py:4523) |
| `longitud_proteccion_salida` | `None` | **vacío — completa el diseño de la Fase 6** | — | [CA:4540](src/criterios_adoptados.py:4540) |
| `angulo_aletas` | `None` | **vacío** | — | [CA:4575](src/criterios_adoptados.py:4575) |
...
2. ~~**`D_MAX` lleva la palabra "VERIFICAR" escrita en el propio código**~~ **YA NO: `D_MAX` salió de `constantes_normativas.py`** (ver la fila de arriba). La nota que cuenta la retirada, con la marca "VERIFICAR" transcrita, vive hoy en el bloque de `D_INICIO`
   ...
   advertencia en `D_max_catalogo` ([CA:3422](src/criterios_adoptados.py:3422)). Los tres topes
...
`cobertura_minima_aashto` `[C]` ([CA:3551](src/criterios_adoptados.py:3551)), la transcripción de la
